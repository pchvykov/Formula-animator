...
Equation = function(latex, paper, coord){
	...
	var create_node = function(inp, parent_set){
		...
		text_object.id = i; //Create a custom 'ID' attribute which saves the type of node and its ID
	...
	this.animate_from = function(eq_old, transf){
	    ...
	    //Create an array with all ids of the expression being moved - for distribution
		var d_ids = [];
		this.forEl(function(el){d_ids.push(el.id)}, R_old.getById(eq_old.get_R_id(transf.get('top'))).parent);
		//console.log(d_ids);
		//console.log(elt.id);
	...
	//copies the whole equation structure, but does not display the new one or set the gui for it.
	this.copy = function(){
	...
	this.distribute = function(holdid,dropid, map){
        ...
        if (results._len == 0){console.error('no options found!')}
