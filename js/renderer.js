...
function scan_tree(input, paper) {
	...
	var visit_children = function(node, idx, pset){
		if (node.n_children() == 0){
			term=create_node(node, pset, paper);
			pset.push(term);
		}
		else{
			var cset = paper.set();
			...
			node.foreach_child(function(child, cidx){
				visit_children(child,cidx,cset);
			})
		}
	}
	visit_children(node, 0, paper.set());
}
...
function display_equation(parent_set,origin)
{
	...
	if (parent_set.length == 1) //Only display if you've hit a terminal node. A terminal node is a set which has only one element. A single element is not explicitly defined as a set is NOT a set and the .length method is not defined for it
	{
		...
		parent_set[0].attr({text: toUnicodeCharacter(parent_set[0].code), x: (origin[0] + offset), y: origin[1], "font-size": font_size});
		var new_origin = [(parent_set[0].getBBox().x2), origin[1]];
		return new_origin;
	}
	else //Set is not a terminal node
	{
		// console.log(parent_set[0].code)
		if(parent_set[0].code == '('){
			parent_set[0].attr({text: parent_set[0].code, x: (origin[0] + offset), y: origin[1], "font-size": font_size}); //Display middle (or father) element
			var origin_right = [(parent_set[0].getBBox().x2-offset/2),parent_set[0].attr("y")];
			var right_paren = display_equation(parent_set[1], origin_right);
			parent_set[2].attr({text: parent_set[2].code, x: (right_paren[0] + offset/2), y: right_paren[1], "font-size": font_size}); //Display middle (or father) element
			...
			// console.log(parent_set[0].id)
			parent_set[0].attr({text: parent_set[0].code, x: (coordinates_of_left_element[0] + offset), y: coordinates_of_left_element[1], "font-size": font_size}); //Display middle (or father) element
			var origin_right = [parent_set[0].getBBox().x2, parent_set[0].attr("y")];
